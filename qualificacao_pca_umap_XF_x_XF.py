...
def feature_selection(X_train, y_train, X_test, model_type_selection, number_components):
    ...
    elif (model_type_selection=='PCA'):
        #pca red. features
        reduction = decomposition.PCA(n_components=number_components)
        components_train = reduction.fit_transform(X_train)
        components_test = reduction.fit_transform(X_test)
        
    elif (model_type_selection=='UMAP'):
        #umap red. features
        reduction = umap.UMAP(n_neighbors=20, min_dist=0.1, n_components=number_components, metric='euclidean')
        components_train = reduction.fit_transform(X_train)
        components_test = reduction.fit_transform(X_test)
...
for number_reduce_components in number_reduce_components_list:
    ...
    for model_type in model_type_list:
        ...
        for model_dimension_reduction in model_reduction_dim_list:
            ...
            print('Reduce/Selection features '+model_type[0]+"_"+model_dimension_reduction+"_"+str(number_reduce_components)+'f')
            features_model01_red, time_reduction_model01 = dimensinality_reduction(model_dimension_reduction, number_reduce_components,features_model01,scaled_feat_reduction)
            ...
            print("Reduce/Selection: "+model_dimension_reduction+" time --> "+"{0:.4f}".format(time_reduction) + " \n")
            ...
            for index, [train, test] in enumerate(kf.split(df)):
                ...
                #gera dataset's 
                #model 01
                dataset_train_md01, dataset_train_label_md01, dataset_test_md01, dataset_test_label_md01 = gen_dataset(features_model01_red, labels, train, test) 
                
                #model 02
                dataset_train_md02, dataset_train_label_md02, dataset_test_md02, dataset_test_label_md02 = gen_dataset(features_model02_red, labels, train, test) 
